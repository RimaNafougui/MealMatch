...
export function GenerateConfig({
  ...
  const [config, setConfig] = useState<MealPlanConfig>({
    ...
    end_date: initialConfig.end_date ?? format(addDays(new Date(), 6), "yyyy-MM-dd"),
  ...
  const addAvoidIngredient = () => {
    ...
    if (!current.includes(val)) update({ avoid_ingredients: [...current, val] });
    setAvoidInput("");
  };

  const removeAvoidIngredient = (item: string) => {
    update({ avoid_ingredients: (config.avoid_ingredients ?? []).filter((i) => i !== item) });
  };
    ...
    <div className="flex flex-col gap-4">
        ...
        <CardBody className="gap-4 p-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col gap-1.5">
              <label className="text-xs text-foreground/50 font-medium">Début</label>
              <input
                type="date"
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs text-foreground/50 font-medium">Fin</label>
              <input
                type="date"
                value={config.end_date ?? format(addDays(new Date(), 6), "yyyy-MM-dd")}
                min={config.start_date ?? todayStr}
                max={maxEndDate}
                onChange={(e) => handleDateChange(config.start_date ?? todayStr, e.target.value)}
                className="w-full px-3 py-2.5 rounded-xl border-2 border-divider bg-content2 text-sm font-medium focus:border-primary focus:outline-none transition-colors cursor-pointer"
              />
            </div>
          </div>
          ...
          {(config.avoid_ingredients ?? []).length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {(config.avoid_ingredients ?? []).map((item) => (
                ...
                >
                  {item}
                  <button onClick={() => removeAvoidIngredient(item)} className="hover:opacity-70">
                    <X size={10} />
                  </button>
                </span>
              ))}
      ...
      <Card className="border border-divider bg-content1">
        <CardBody className="p-5">
          <button
            onClick={() => update({ allow_repetitions: !config.allow_repetitions })}
            className="flex items-center justify-between w-full"
          >
            <div className="flex items-center gap-3">
              <div className={`p-2 rounded-lg ${config.allow_repetitions ? "bg-success/10" : "bg-default-100"}`}>
                <RefreshCcw size={15} className={config.allow_repetitions ? "text-success" : "text-foreground/40"} />
              </div>
              <div className="text-left">
                <p className={`text-sm font-semibold ${config.allow_repetitions ? "text-success" : "text-foreground/70"}`}>
                  Mode meal prep
                </p>
            ...
            <div className={`relative w-10 h-5.5 rounded-full transition-colors ${config.allow_repetitions ? "bg-success" : "bg-default-200"}`}>
              <span
      ...
      <div className="flex items-center gap-2 flex-wrap px-1">
        <Chip color="primary" variant="flat" size="sm">{config.days_count} jours</Chip>
        <span className="text-foreground/40 text-sm">×</span>
        <Chip color="primary" variant="flat" size="sm">{config.meals_per_day} repas/jour</Chip>
        <span className="text-foreground/40 text-sm">=</span>
